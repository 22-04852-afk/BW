...
if (!$itemCode || !$itemName || $quantity <= 0) {
    ...
    echo json_encode(['error' => 'Invalid item code, name, or quantity']);
    exit;
}

// Create a stock addition record in delivery_records
...
if ($checkResult && $checkResult->num_rows > 0) {
    ...
    $updateSql = "UPDATE delivery_records 
                  SET quantity = {$newQuantity}, notes = '{$notesEscaped}'
                  WHERE id = {$recordId}";
    ...
    if ($conn->query($updateSql)) {
        ...
        echo json_encode(['error' => 'Failed to update stock: ' . $conn->error]);
    }
} else {
    ...
    if ($conn->query($insertSql)) {
        ...
        http_response_code(500);
        echo json_encode(['error' => 'Failed to add stock: ' . $conn->error]);
    }
}
